...
A Go-based picture frame application that allows uploading and displaying media content. The application consists of a server for media uploads and a client for displaying content in kiosk mode.
...
- **Media Support**: Images (JPG, PNG, GIF), videos (MP4, AVI, MOV, WEBM), and text messages
...
For the client to work properly, you need one of the following browsers installed:
- `chromium-browser`
- `google-chrome`
- `firefox`
...
Media files are stored in the `data/<frame-id>/` directory, with metadata stored in JSON format for efficient retrieval and display.